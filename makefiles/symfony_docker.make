...
shell:
	@printf "${INFO}docker-compose exec php sh${EOL}"
	@docker-compose exec php sh

## Open in default browser
open:
	@printf "${INFO}nohup xdg-open `docker inspect $(shell basename ${CURDIR}) --format 'http://{{.NetworkSettings.Networks.tango.IPAddress}}/api/docs' 2>/dev/null` >/dev/null 2>&1${EOL}"
