...
help:
	...
	@printf "${SUCCESS}  %-{{ PADDING }}s${INFO} %s${NL}" "login"     "$(shell whoami)"
	@printf "${SUCCESS}  %-{{ PADDING }}s${INFO} %s${NL}" "system"    "$(shell uname -s)"
	@printf "${SUCCESS}  %-{{ PADDING }}s${INFO} %s${NL}" "httpduser" "${HTTPDUSER}"
	@printf "${SUCCESS}  %-{{ PADDING }}s${INFO} %s${NL}" "port"      "${port}"
	@printf "${SUCCESS}  %-{{ PADDING }}s${INFO} %s${NL}" "php"       "${PHP_VERSION}"
	@printf "${SUCCESS}  %-{{ PADDING }}s${INFO} %s${NL}" "Symfony"   "${VERSION}"
	@printf "${NL}"
